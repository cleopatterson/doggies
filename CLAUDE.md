...
6. **Claude synthesis** — sends fixture + previous match + Kennel digests to `claude-opus-4-7` with a strict-JSON prompt. Returns `kennelTipLean`, exactly 1 `kennelTipQuote` (the spiciest), exactly 2 `debates` constrained to observably-resolvable coaching calls, a weekly `trivia` block (question + 4 options + `correctIndex` + explainer), exactly 1 `recap` question sourced **directly from the prevMatch digest** (try scorers, top performers, key stats — strict instructions to never invent and to keep the stat value out of the option labels), and a `washup` blob (headline, vibe, kennelMood, kennelSummary, kennelHotTakes). Each `debate`, quote, and hot take includes a `threadSlug` that gets resolved to a full Kennel URL after Claude returns. The `oddsDrivers` block was removed — felt like AI filler.
   - **Trivia research doc + no-repeat** — [trivia-research.md](trivia-research.md) (repo root) is the trivia source. Everything above its `## Asked so far` heading is hand-curated research material fed into the prompt ("build the question from this doc"); the section below it is an auto-appended log of every question shipped (one `- Rd N (date): "question" — answer: …` line per round), and the FULL list goes into the prompt as a hard never-repeat rule — cumulative across the season, not just last week. `generate.js` appends to the log after each successful write (idempotent — skips if the question's already logged), and the GitHub Action commits the file alongside `data.json`. If the doc is missing or still the placeholder stub, the generator soft-fails back to Claude's own Bulldogs knowledge + the old single-prior-question rule. **Trivia is also stable within a round** — if the prior `data.json` is for the same round (Tue → Thu rerun), the existing question is kept rather than regenerated, so a locked answer never has its question swapped underneath it.
